...
const CriterioModal = ({ showModal, closeModal, modalType, selectedItem, onSave, departments }) => {
  const initialState = {
    nome: '',
    codigo: '',
    descricao: '',
  ...
  const handleSubmit = async (e) => {
    ...
    if (isDeleteMode) {
        try {
            ...
            toast.error(err.response?.data?.error || 'Erro ao apagar critério.');
        }
        return;
    }
    if (!validate()) {
    ...
    try {
      ...
      toast.error(err.response?.data?.error || 'Erro ao salvar critério.');
    }
  };
  ...
  const getTitle = () => {
    if (isViewMode) return 'Visualizar Critério';
    if (isDeleteMode) return 'Desativar Critério';
    return modalType === 'create' ? 'Criar Novo Critério' : 'Editar Critério';
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] flex flex-col">
        ...
        <div className="p-6 overflow-y-auto flex-grow">
          <form id="criterio-form" onSubmit={handleSubmit}>{renderContent()}</form>
        </div>
        <div className="p-4 border-t flex justify-end space-x-3">
          ...
          {!isViewMode && (
            <button type="submit" form="criterio-form" className={`px-4 py-2 text-white rounded-md text-sm font-medium ${isDeleteMode ? 'bg-red-600 hover:bg-red-700' : 'bg-blue-600 hover:bg-blue-700'}`}>
