...
import { getSavingsProducts, createSavingsProduct, updateSavingsProduct, deleteSavingsProduct } from '../services/api';
import SavingsProductModal from './SavingsProductModal';
...
const SavingsProducts = () => {
    ...
    const handleSave = async (productData) => {
        try {
            if (selectedProduct) {
                await updateSavingsProduct(selectedProduct.id, productData);
            } else {
                await createSavingsProduct(productData);
            }
            fetchProducts();
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Tem a certeza que quer apagar este produto de poupança?')) {
            try {
                await deleteSavingsProduct(id);
                fetchProducts();
            } catch (error) {
                console.error('Error deleting savings product:', error);
                alert('Erro ao apagar o produto de poupança.');
            }
        }
    };
    ...
    return (
        <div className="p-6">
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-2xl font-semibold">Produtos de Poupança</h2>
                <button 
                ...
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                        <tr>
                            ...
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Frequência Pag.</th>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {products.map((product) => (
                            <tr key={product.id}>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{product.payment_frequency}</td>
