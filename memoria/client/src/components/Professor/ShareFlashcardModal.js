...
const ShareFlashcardModal = ({ flashcard, onClose }) => {
    ...
    const fetchInitialData = useCallback(async () => {
        ...
        try {
            ...
            setProfessorDisciplines(disciplinesRes.data.data || []);
            setSharedWith(sharedRes.data.data || []);

        } catch (err) {
            console.error('Erro ao carregar dados da partilha:', err);
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        toast.promise(
            api.shareFlashcard(flashcard.id, sharedWith),
            {
                loading: 'A guardar...',
                success: () => {
                    setLoading(false);
                    onClose();
                    return 'Flashcard partilhado com sucesso!';
                },
                error: () => {
                    setLoading(false);
                    return 'Ocorreu um erro ao partilhar.';
                }
            }
        );
    };
